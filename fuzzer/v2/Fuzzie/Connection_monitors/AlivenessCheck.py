                ...
                if len(sta_Ip) > 5:
                    ...
                    while responsive == 'notfound' or responsive == '1':
                        if responsive == '1': #look at pingg function, exception has been triggered
                            print('Sleeping 10s because something went really wrong. Check your WNIC')
                            sleep(10)
                        else:
                            print('\n' + bcolors.WARNING + 'Pinging stopped responding' + bcolors.ENDC)
                            input(bcolors.WARNING + 'Get the STA back online and press enter: ' + bcolors.ENDC)
                        responsive = self.pingg(sta_Ip)
                    print("\nSTA is responsive")
                    settings.retrieving_IP = True
                    return sta_Ip
                else: 
                    print('\n' + bcolors.FAIL + 'Could not find the IP of MAC: ' + bcolors.ENDC + self.targeted_STA)
